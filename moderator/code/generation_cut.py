...
You and your neighbor will exchange messages to negotiate how these items will be split. During the conversation, feel free to give personal 
...
"""
    ...
    botA_messages = [{"role": "system", "content": botA_system_msg}]
    botB_messages = [{"role": "system", "content": botB_system_msg}]

    full_chat_logs = dialogue["chat_logs"]
    last_empty_index = -1
    for i in reversed(range(len(full_chat_logs))):
        if not full_chat_logs[i]["task_data"]:
            last_empty_index = i
            break

    # half_length is used to control the dialogue history given. inital message/half dialogue history/all dialogue history
    half_length = last_empty_index+1

    last_speaker = None

    for i in range(half_length):
        speaker_id = full_chat_logs[i]["id"]
        text = full_chat_logs[i]["text"]

        if speaker_id == "mturk_agent_1":
            botA_messages.append({"role": "assistant", "content": text})
            botB_messages.append({"role": "user", "content": text})
            last_speaker = "A"
        else:
            botB_messages.append({"role": "assistant", "content": text})
            botA_messages.append({"role": "user", "content": text})
            last_speaker = "B"
...
output_filename = "negotiation_results_test_for_good_all_cut_last2_second_try.json"
